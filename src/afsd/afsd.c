...
/*
  ...
  *     -vicepaccess Allow direct access to visible fileserver partitions
  *     -osdaccess   Allow direct access to visible OSD partitions
  *
  *  Flags for afsd as command:
  *     -shutdown  Shutdown afs daemons
  *     -check-fspartitions Rescan for visible fileserver partitions
  *     -check-osds         Rescan for visible rxosd partitions
  *---------------------------------------------------------------------------*/
...
int nServerUuids = 0;
struct fsInfo fsInfo[MAXSERVERUUIDS];
struct osdInfo {
    afs_uint32 osd;
    afs_uint32 lun;
    char cell[MAXCELLCHARS];
...
int nOsdIds = 0;
struct osdInfo osdInfo[MAXSERVERUUIDS];
...
static int vicepaccess = 0;     /* enable direct I/O to visible fs
                                 * vicep-partitions */
static int osdaccess = 0;      /* enable direct I/O to visible OSD part. */
...
FindFileserverPartitions(void)
{
    ...
    for (currp = readdir(cdirp); currp; currp = readdir(cdirp)) {
#endif
        if (strncmp(currp->d_name, "vicep", 5) == 0) {
            afs_uint32 lun;
            int symlinklng;
            sprintf(filename, "/%s", currp->d_name);
            symlink[0] = 0;
            symlinklng = readlink(filename, symlink, 256);
            if (symlinklng > 0)
                symlink[symlinklng] = 0;
            if (symlinklng > 64)  {
                fprintf(stderr, "Target of symbolic link %s %s is too long, ignored\n",
                                filename, symlink);
                symlink[0] = 0; /* symbolic link too long */
            }
            sprintf(filename, "/%s/partid", currp->d_name);
            fd = open(filename, O_RDONLY, 0);
            if (fd >= 0) {
                sprintf(filename, "/%s", currp->d_name);
                if ((read(fd, (char *)&vsn, sizeof(vsn))) != sizeof(vsn))
                    goto bad_partid;
                vsn.magic = ntohl(vsn.magic);
                vsn.version = ntohl(vsn.version);
                if (vsn.magic != PARTIDMAGIC)
                    goto bad_partid;
                if (vsn.version != 1)
                    goto bad_partid;
                if (read(fd, (char *)&lun, sizeof(lun)) != sizeof(lun))
                    goto bad_partid;
                fsInfo[nServerUuids].lun = ntohl(lun);
                if (read(fd, (char *)&fsInfo[nServerUuids].uuid,
                  sizeof(struct afsUUID)) != sizeof(struct afsUUID))
                    goto bad_partid;
                afs_ntohuuid(&fsInfo[nServerUuids].uuid);
                if (read(fd, (char *)&fsInfo[nServerUuids].cell,
                  MAXCELLCHARS) != MAXCELLCHARS)
                    goto bad_partid;
                fsInfo[nServerUuids].path[0] = '/';
                if (symlink[0])
                    strncpy(fsInfo[nServerUuids].path, symlink, 64);
                else
                    strncpy(fsInfo[nServerUuids].path, filename, 64);
                nServerUuids++;
                if (afsd_verbose)
                    printf("afsd: fileserver partition found '/%s'\n", currp->d_name);
        bad_partid:
                close(fd);
            }
            sprintf(filename, "/%s/osdid", currp->d_name);
            fd = open(filename, O_RDONLY, 0);
            if (fd >= 0) {
                afs_uint32 timestamp;
                sprintf(filename, "/%s", currp->d_name);
                if (read(fd, (char *)&osdInfo[nOsdIds].osd, 4) == 4
                  && read(fd, (char *)&timestamp, 4) == 4
                  && read(fd, (char *)&osdInfo[nOsdIds].cell, MAXCELLCHARS)
                  == MAXCELLCHARS) {

                    osdInfo[nOsdIds].osd = ntohl(osdInfo[nOsdIds].osd);
                    osdInfo[nOsdIds].lun = volutil_GetPartitionID(currp->d_name);
                    if (symlink[0])
                        strncpy(osdInfo[nOsdIds].path, symlink, 64);
                    else
                        strncpy(osdInfo[nOsdIds].path, filename, 64);
                    if (afsd_verbose)
                        printf("afsd: partition for osd %u found '/%s'\n",
                                osdInfo[nOsdIds].osd, currp->d_name);
                    nOsdIds++;
                }
                close(fd);
            }
...
mainproc(struct cmd_syndesc *as, void *arock)
{
    ...
    if (as->parms[40].items) {
        ...
        for (i = 0; i < nServerUuids; i++) {
            afsd_call_syscall(AFSOP_SERVERUUID, &fsInfo[i].uuid, &fsInfo[i].cell,
                        &fsInfo[i].path, fsInfo[i].lun);
        }
        exit(0);
    }
    if (as->parms[41].items) {
        /* -check-osds */
        FindFileserverPartitions();
        for (i=0; i<nOsdIds; i++) {
            afsd_call_syscall(AFSOP_OSDID,
                        &osdInfo[i].cell,
                        &osdInfo[i].path,
                        osdInfo[i].osd,
                        osdInfo[i].lun);
...
# endif /* !AFS_SUN510_ENV */
    ...
    if (vicepaccess || osdaccess) {
        ...
        if (osdaccess) {
            for (i=0; i<nOsdIds; i++) {
                afsd_call_syscall(AFSOP_OSDID, &osdInfo[i].cell,
                            &osdInfo[i].path,
                            osdInfo[i].osd,
                            osdInfo[i].lun);
            }
...
afsd_init(void)
{
    ...
    cmd_AddParm(ts, "-rxmaxfrags", CMD_SINGLE, CMD_OPTIONAL,
		...
		" per Rx packet");
    cmd_AddParm(ts, "-vicepaccess", CMD_FLAG, CMD_OPTIONAL,
                "Enable direct I/O to visible fileserver-partitions");
    cmd_AddParm(ts, "-osdaccess", CMD_FLAG, CMD_OPTIONAL,
                "Enable direct I/O to visible OSD-partitions");
    cmd_AddParm(ts, "-check-fspartitions", CMD_FLAG, CMD_OPTIONAL,
                "Check for visible fileserver partitions and exit");
    cmd_AddParm(ts, "-check-osds", CMD_FLAG, CMD_OPTIONAL,
                "Check for visible OSD partitions and exit");
}
