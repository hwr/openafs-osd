...
extern afs_uint32 fakeStripes;
extern afs_uint32 logFakeStripes;
...
DECL_PIOCTL(PFsCmd)
{
	...
	do {
		...
		if (!code && Inputs->command == CMD_REPLACE_OSD)
                    tvc->f.states &= ~CStatd;
#if 0		/* don't know why this crashes at afs_conn.c:541 (refcnt<0) */
...
/*  AFS_STATCNT(PSetHomeCell); */
    ...
    if (afs_pd_getStringPtr(ain, &name) == 0) {
        if (!afs_osi_suser(*acred))
            return EACCES;
        tcell = afs_GetCellByName(name, READ_LOCK);
        if (!tcell)
            return EINVAL;
        afs_SetPrimaryCell(tcell->cellName);
        afs_PutCell(tcell, READ_LOCK);
    }
#endif
    return 0;
