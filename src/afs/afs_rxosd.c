...
rxosd_Destroy(void **r, afs_int32 code)
{
    ...
    *r = NULL;
	...
	RX_AFS_GUNLOCK();
	if (v->writing) {
	    for (code2 = RX_CALL_TQ_BUSY, busy = 0; code2 == RX_CALL_TQ_BUSY; busy++)
...
rxosd_storeWrite(void *r, char *abuf, afs_uint32 length, afs_uint32 *byteswritten)
{
    ...
    *byteswritten = 0;
	...
	/* extend expiration for async store request */	
 	RX_AFS_GUNLOCK();
	for (code = RX_CALL_TQ_BUSY, busy = 0; code == RX_CALL_TQ_BUSY; busy++)
	...
	afs_int32 code1;
        RX_AFS_GUNLOCK();
	for (code1 = RX_CALL_TQ_BUSY, busy = 0; code1 == RX_CALL_TQ_BUSY; busy++)
