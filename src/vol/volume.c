...
VInitAttachVolumes(ProgramType pt)
{
    if (VInit == 1) {
        ...
        VOL_UNLOCK;
    } else { /* called from FiveMinuteCheck */
        if (pt == fileServer) {
	    struct DiskPartition64 *diskP;
	    /* Attach all the volumes in this partition */
	    for (diskP = DiskPartitionList; diskP; diskP = diskP->next) {
		if (diskP->flags & PART_NEWLY_MOUNTED) {
	            int nAttached = 0, nUnattached = 0;
		    diskP->flags &= ~PART_NEWLY_MOUNTED;
	            osi_Assert(VAttachVolumesByPartition(diskP,
			       &nAttached, &nUnattached) == 0);
		}
	    }
	}
    }
...
VInitAttachVolumes(ProgramType pt)
{
	...
	if (VInit != 1) {
	    for (parts = 0, diskP = DiskPartitionList; diskP; diskP = diskP->next) {
	        if (diskP->flags & PART_NEWLY_MOUNTED)
		    parts++;
	    }
	    if (!parts)
	...
	for (parts = 0, diskP = DiskPartitionList; diskP; diskP = diskP->next) {
	    ...
	    if (VInit != 1 && !(diskP->flags & PART_NEWLY_MOUNTED))
		continue;
	    diskP->flags &= ~PART_NEWLY_MOUNTED;
        ...
        Volume * vp, int isbusy, int mode, int *acheckedOut)
{
	...
	*ec = VSALVAGE;
    ...
    if (VCanUnsafeAttach() && VolumeWriteable(vp) && V_inUse(vp) == fileServer 
      && !V_needsSalvaged(vp)) {
