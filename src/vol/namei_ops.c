 ...
 */
static int
SetOGM(int fd, int parm, int tag, int special)
{
 ...
 */
Inode
namei_icreate_open(IHandle_t * lh, char *part, int p1, int p2, int p3, int p4,
                        int *open_fd)
{
    namei_t name;
    int fd = -1;
...
namei_icreate(IHandle_t * lh, char *part, int p1, int p2, int p3, int p4)
{
    ...
    ino = namei_icreate_open(lh, part, p1, p2, p3, p4, 0);
    return ino;
}
...
#if defined(BUILDING_RXOSD) && defined(AFS_RXOSD_SPECIAL)
    fd = h->ih_ops->open(name.n_path, O_RDWR, 0666);
#else
...
namei_dec(IHandle_t * ih, Inode ino, int p1)
{
	...
	if (count == 0) {
	    IHandle_t *th;
#if defined(BUILDING_RXOSD) && (defined(AFS_AIX53_ENV) || defined(AFS_DCACHE_SUPPORT))
 ...
 * different version of the file (or a different uniquifer) for the BK volume.
 * Then one column would be holding the link count of 2 for the RW and RO
 * and a different column would hold the link count of 1 for the BK volume.
...
namei_GetLinkCount2(FdHandle_t * h, Inode ino, int lockit, int fixup, int nowrite)
{
    ...
    if (rc == 0 && fixup) {
        struct stat st;
        if (fstat(h->fd_fd, &st) || st.st_size >= offset+length)
	   goto bad_getLinkByte;
...
GetFreeTag(IHandle_t * ih, int vno)
{
    ...
    nBytes = read(fdP->fd_fd, buf, length);
    if (!shared)
        row = shortrow;
    if (nBytes != length) {
	if (nBytes != 0)
    ...
    if (!shared)
        shortrow = row;
    if (write(fdP->fd_fd, buf, length) != length) {
	goto badGetFreeTag;
    }
    FDH_SYNC(fdP);
    FLOCK(fdP, LOCK_UN);
		     ...
		     afs_uint32 singleVolumeNumber, void *rock)
{
	...
	while ((dp1 = readdir(dirp1))) {
	    if (*dp1->d_name == '.')
		continue;
	    if (DecodeInode(path1, dp1->d_name, &info, &myIH) < 0)
		continue;
...
#ifdef FSSYNC_BUILD_CLIENT
    namei_t n;
    char dir_name[512], oldpath[512], oldheader[128], newpath[512];
    char smallName[64];
    char largeName[64];
    char infoName[64];
    char osdName[64];
...
# endif /* AFS_DEMAND_ATTACH_FS */
    ...
    *newId = ih->ih_vid;
    ...
    namei_HandleToName(&n, &t_ih);
    (void)afs_snprintf(newpath, sizeof newpath, "%s/%s", dir_name, osdName);
    fd = afs_open(newpath, O_RDWR, 0);
    if (fd < 0) {
    ...
    h.linkTable_hi = h.id;	/* should not be necessary */
#ifdef AFS_RXOSD_SUPPORT
