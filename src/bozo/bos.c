...
DoSalvage(struct rx_connection * aconn, char * aparm1, char * aparm2,
	  ...
	  char * atmpDir, char * orphans, int dafs, int debug, int nowrite,
	  int force, int oktozap, int rootfiles, int salvagedirs,
	  int blockreads)
{
    afs_int32 code;
    char *parms[6];
...
SalvageCmd(struct cmd_syndesc *as, void *arock)
{
	...
	rc = DoSalvage(tconn, NULL, NULL, outName, showlog, parallel, tmpDir,
		       orphans, dafs, debug, nowrite, force, oktozap, 
		       rootfiles, salvagedirs, blockreads);
	if (curGoal == BSTAT_NORMAL) {
	...
	rc = DoSalvage(tconn, as->parms[1].items->data, NULL, outName,
		       showlog, parallel, tmpDir, orphans, dafs, debug,
		       nowrite, force, oktozap, rootfiles, salvagedirs,
		       blockreads);
	if (curGoal == BSTAT_NORMAL) {
	    printf("bos: restarting '%s'.\n", serviceName);
	    code = BOZO_SetTStatus(tconn, serviceName, BSTAT_NORMAL);
...
main(int argc, char **argv)
{
    ...
    cmd_AddParm(ts, "-oktozap", CMD_FLAG, CMD_OPTIONAL,
		"Give permission to destroy bogus file residencies/volumes - debugging flag");
    cmd_AddParm(ts, "-rootfiles", CMD_FLAG, CMD_OPTIONAL,
		"Show files owned by root - debugging flag");
