...
static struct afsconf_servPair serviceTable[] = {
    ...
    {"afsrxosd", NULL, 7011,},  /* rxosd OSD for OpenAFS + object storage */
    {"afsosddb", NULL, 7012,},	/* osddb database for OpenAFS + object storage */
...
afsconf_SetCell(struct afsconf_dir *adir, char *cell)
{
    ...
    strcompose(path, 128, adir->name, "/", AFSDIR_THISCELL_FILE, NULL);
    strcpy(newpath, path);
    strcat(newpath, ".tmp");
    fd = open(newpath, O_CREAT | O_RDWR, 0644);
    if (fd < 0)
       return EIO;
    if (write(fd, cell, strlen(cell)) != strlen(cell)) {
       close(fd);
       unlink(newpath);
       return EIO;
    }
