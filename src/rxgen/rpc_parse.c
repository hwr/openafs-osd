...
check_proc(definition * defp, token * tokp, int noname)
{
    ...
    scan5(TOK_SLOW, TOK_SPLIT, TOK_MULTI, TOK_EQUAL, TOK_SEMICOLON, &tok);
    if (tok.kind == TOK_SLOW) {
	defp->pc.slow_flag = 1;
...
cs_ProcMarshallInParams_setup(definition * defp, int split_flag)
{
    ...
    /*
     * For slow rpcs set RX_CALL_LONG_RUNNING in z_call->flags
     * to avoid idleDead while the rpc is still running.
     */
    if (!split_flag && defp->pc.slow_flag == 1) {
        f_print(fout, "\tz_call->flags |= RX_CALL_LONG_RUNNING;\n");
    }
