			...
			struct VnodeDiskObject *parVnode) 
{
    ...
    fdP = IH_OPEN(vol->vnodeIndex[class].handle);
    if (!fdP) {
	Log("Couldn't open %s Index of volume " AFS_VOLID_FMT "u\n", 
		class ? "small":"large", afs_printable_VolumeId_lu(V_id(vol)));
	code = EIO;
	goto Bad_Copy;
    }
	...
	if (e->flag) {
		...
		Log("Couldn't read in %s Index of volume %" AFS_VOLID_FMT " at offset %"
		    AFS_UINT64_FMT "\n", class ? "small":"large",
		...
		vnode->cloned = 0;
		    ...
		    Log("Couldn't write in %s Index of volume %" AFS_VOLID_FMT " at offset %"
			AFS_UINT64_FMT "\n", class ? "small":"large",
			afs_printable_VolumeId_lu(V_id(vol)), offset);
		    code = EIO;
		    goto Bad_Copy;
	    ...
	    if (e->flag & NEEDED && e->vN != where) {
	        ...
	        *blocks += (size + 0x3ff) >> 10;
		    ...
		    if (code) {
                        Log("vol_split: namei_replace_file_by_hardlink for %" AFS_VOLID_FMT ".%u.%u failed with %d\n", 
                            afs_printable_VolumeId_lu(V_id(newvol)), e->vN,
			    vnode->uniquifier, code);
                        code = EIO;
                        goto Bad_Copy;
                    }
		    VNDISK_SET_INO(vnode, newino);
		    ...
		    if (code) {
                        Log("vol_split: op_split_objects for %" AFS_VOLID_FMT ".%u.%u failed with %d\n", 
                            afs_printable_VolumeId_lu(V_id(newvol)), e->vN,
			    vnode->uniquifier, code);
                        code = EIO;
                        goto Bad_Copy;
		...
		vnode->cloned = 0;
		    ...
		    Log("Couldn't write in %s Index of volume %" AFS_VOLID_FMT " to offset %"
			AFS_UINT64_FMT "\n", class ? "small":"large",
			afs_printable_VolumeId_lu(V_id(newvol)), offset);
		    code = EIO;
		    goto Bad_Copy;
	        }
	    }
	...
	if (FDH_PREAD(fdP, vnode, vcp->diskSize, offset) != vcp->diskSize) {
	    Log("Couldn't read in large Index of old volume %" AFS_VOLID_FMT " at offset %"
		AFS_UINT64_FMT "\n", afs_printable_VolumeId_lu(V_id(vol)), offset);
	    code = EIO;
	    goto Bad_Copy;
	}
		...
		afs_uint32 vN,  struct VnodeDiskObject *vd, char *name)
{
	...
	Log("splitvolume: couldn't delete directory entry for %s in %" AFS_VOLID_FMT ".%u.%u, code = %d\n",
			name, afs_printable_VolumeId_lu(V_id(vol)), vN,
			parent->uniquifier, code);
	return code;
    }
	...
	Log("split volume: couldn't write vnode for parent directory " AFS_VOLID_FMT "u.%u.%u\n", 
	     ...
	     afs_uint32 where, afs_int32 verbose)
{
    ...
    if (code) {
	sprintf(m->line, 
		"(step 1) ExtractVnodes failed for %" AFS_VOLID_FMT " for directories with code %d\n",
    ...
    if (code) {
	sprintf(m->line, 
		"(step 2) findname for %u in directory %" AFS_VOLID_FMT ".%u.%u. failed with %d\n",
		where, afs_printable_VolumeId_lu(V_id(vol)), parent,
		parVnode->uniquifier, code); 
        inform(m, 1);
	return code;
    }
    ...
    if (code) {
	sprintf(m->line, 
		"(step 3) FindVnodes for directories in vol %" AFS_VOLID_FMT " failed with %d\n",
			afs_printable_VolumeId_lu(V_id(vol)), code);
        inform(m, 1);
	return code;
    }
    ...
    if (code) {
	sprintf(m->line, 
		"(step 4) ExtractVnodes files in vol %" AFS_VOLID_FMT " failed with %d\n",
		afs_printable_VolumeId_lu(V_id(vol)), code);
        inform(m, 1);
	return code;
    }
    ...
    if (code) {
        sprintf(m->line,
                "(step 5) FindVnodes for files in vol %" AFS_VOLID_FMT " failed with %d\n",
                        afs_printable_VolumeId_lu(V_id(vol)), code);
        inform(m, 1);
        return code;
    }
    ...
    if (code) {
	sprintf(m->line, "(step 6) copyVnodes files from %" AFS_VOLID_FMT " to %" AFS_VOLID_FMT " failed with code %d\n",
	    afs_printable_VolumeId_lu(V_id(vol)),
	    afs_printable_VolumeId_lu(V_id(newvol)), code);
    ...
    if (code) {
	sprintf(m->line, "(step 7) copyVnodes directories from %" AFS_VOLID_FMT " to %" AFS_VOLID_FMT " failed with code %d\n",
	    afs_printable_VolumeId_lu(V_id(vol)),
	    afs_printable_VolumeId_lu(V_id(newvol)), code);
        inform(m, 1);
	return code;
    ...
    if (code) {
        sprintf(m->line, "(step 8) update of new volume %" AFS_VOLID_FMT " failed with %d\n",
                afs_printable_VolumeId_lu(V_id(newvol)), code);
        inform(m, 1);
        return code;
    }
    ...
    if (code) {
        sprintf(m->line, "(step 9) createMountpoint %s in vol %" AFS_VOLID_FMT " for vol %" AFS_VOLID_FMT " failed with %d\n",
                name, afs_printable_VolumeId_lu(V_id(vol)),
		afs_printable_VolumeId_lu(V_id(newvol)), code);
        inform(m, 1);
        return code;
    }
    ...
    if (code) {
        sprintf(m->line, "(step 11) update of old volume %" AFS_VOLID_FMT " failed with %d\n",
                afs_printable_VolumeId_lu(V_id(vol)), code);
        inform(m, 1);
        return code;
    }
    ...
    inform(m, 0);
    sprintf(m->line,
        "Volume %" AFS_VOLID_FMT " successfully split at vnode %u '%s': %u dirs and %u files in new vol %" AFS_VOLID_FMT "\n",
                afs_printable_VolumeId_lu(V_id(vol)), where, name, dirsNeeded,
		filesNeeded, afs_printable_VolumeId_lu(V_id(newvol)));
    inform(m, 1);
    m->line[0] = 0;
    m->line[1] = 0;
    m->line[2] = 0;
