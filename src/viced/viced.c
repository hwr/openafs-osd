...
FiveMinuteCheckLWP(void *unused)
{
    ...
    char tbuffer[32];
    time_t now, wakeup;
    afs_int32 sleepseconds, seconds;
...
SetupVL(void)
{
	...
	/*
	 * Find addresses we are supposed to as per the netrestrict 
	...
	if (code < 0) {
	    ViceLog(0, ("Can't any valid addresses: %s\n", reason));
	    exit(1);
	}
...
InitVL(void)
{
    ...
    /* Read or create the sysid file and the fileserver's
